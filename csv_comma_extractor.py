...
Options:
    ...
    --layout      'vertical' (one row per line) or 'horizontal' (one row per match) [default: horizontal]
...
Examples:
    ...
    python csv_comma_extractor.py --files file1.csv file2.txt --layout vertical --output results.xlsx
"""
...
def write_vertical(all_blocks: list, output_path: Path):
    ...
    for blk in all_blocks:
        ...
        for line_num, raw, is_match in blk["rows"]:
            ...
            if line_num - 1 < mi:
                role = f"Context before ({mi - (line_num - 1)})"
                fill = CTX_B_FILL
            elif line_num - 1 == mi:
                role = "Match"
                fill = MATCH_FILL
            else:
                role = f"Context after ({(line_num - 1) - mi})"
                fill = CTX_A_FILL
    ...
    ws.freeze_panes = "A2"
    wb.save(output_path)
    print(f"Saved (vertical): {output_path}  ({ws.max_row - 1} rows)")
...
def write_horizontal(all_blocks: list, before: int, after: int, output_path: Path):
    ...
    for blk in all_blocks:
        match_line_num = blk["rows"][blk["match_idx"]][0]
        data = [blk["file"], match_line_num]
    ...
    ws.freeze_panes = "C2"
    wb.save(output_path)
    print(f"Saved (horizontal): {output_path}  ({ws.max_row - 1} match rows)")
...
def main():
    ...
    print(f"Files: {len(file_paths)}  |  search='{args.search or '(any comma line)'}'  "
          f"|  context={args.before}+{args.after}  |  layout={args.layout}")
    print("-" * 65)
    ...
    for fp in file_paths:
        try:
            ...
            print(f"  Warning: {fp.name}: {e}"); continue

        match_indices = find_matches(lines, args.search)
        total_matches += len(match_indices)
        print(f"  {fp.name}: {len(match_indices)} match(es)")
        ...
        for mi in match_indices:
            ...
            # local index of match inside rows[]
            local_idx = mi - start
            all_blocks.append({
                "file": fp.name,
                "rows": rows,
    ...
    if not all_blocks:
        print("No matches found. Try a different --search term or leave it blank.")
        return

    out = Path(args.output)
    if args.layout == "horizontal":
        write_horizontal(all_blocks, args.before, args.after, out)
    else:
        write_vertical(all_blocks, out)
